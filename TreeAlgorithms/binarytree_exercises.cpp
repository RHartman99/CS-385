...
struct Node
{
    ...
    Node(int level, int data) : parent{nullptr}, left{nullptr}, right{nullptr},
                                level{level}, data(data) {}

    Node(Node *parent, int level, int data) : parent{parent}, left{nullptr}, right{nullptr},
                                              level{level}, data{data} {}
};
...
{
    Node *root = new Node(0, data[length / 2])
        create_bst_helper() return NULL;
}
...
int main(int argc, const char *argv[])
{
    ...
    cout << "Lowest common ancestor: " << common_ancestor->data << endl
         << endl;
